...
src/kitchenowl_mcp/
  ...
  tools/
    ...
    shopping.py  get_list, add_items, update_item, clear_checked
...
- **Agent:** Anthropic Messages API, wired directly to the same 15 tool functions `/mcp` registers (in-process calls, not through MCP's JSON-RPC transport). Needs its own `ANTHROPIC_API_KEY` — separate, metered billing, not a reuse of any personal claude.ai/Claude subscription.
